...
| Configuration Governance   | The data that is generated by the service:<ul><li>Does not contain any personally identifiable information. </li><li>Is sent over a TLS 1.2+ protected channel by using {{site.data.keyword.cloud_notm}} Internet Services (powered by Cloudflare). By using Cloud Internet Services there is DDoS protection, a Web Application Firewall, and other network level security mechanisms in place. </li><li>Is encrypted while at rest and in transit to a provider-managed Cloudant database (configuration data) or a provider-managed {{site.data.keyword.cloud_notm}} Databases Elastic Search (results data). </li><li>Is securely replicated in US-South and US-East regions. </li><ul> </br>**Note:** Results data is kept for 7 days before it is deleted. |
| Security Insights | The data that is generated by the service: <ul><li>Does not contain any personally identifiable information. </li><li>Is encrypted at rest and in transit. </li><li>Is stored in a Cloudant database within each region. </li><li>Is backed up daily to Cloud Object Storage. </li><li>Is automatically recovered by IBM in the event of a failure.</li></ul> |
{: caption="Table 1. Data storage information for {{site.data.keyword.compliance_short}}" caption-side="top"}

When you work with the {{site.data.keyword.compliance_short}}, you provide credentials that allow a collector to access and scan your IT resources. The {{site.data.keyword.compliance_short}} securely stores your credentials and prevents anyone from accessing them - including IBM. At no time are your credentials available in clear text while stored or used by the service. For more information, see [Managing collectors](/docs/security-compliance?topic=security-compliance-collector).
{: note}
