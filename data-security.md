...
## How your data is stored and encrypted in {{site.data.keyword.compliance_short}}
{: #data-storage}

When you work with the {{site.data.keyword.compliance_short}}, results data that is generated by the service, findings, and the credentials that you provide are stored and encrypted.
...
Depending on the component of the {{site.data.keyword.compliance_short}} that you're working with, data is stored and encrypted differently.

#### Posture Management
{: #posture-results-security}

The configuration information and metadata that is gathered by the service for Posture Management:
...
* Is stored by the service for 30 days (configuration data) or 31 days (analytics data) in an encrypted database that is provided by the {{site.data.keyword.cloud_notm}} Database Service.
...
* Is sent over a TLS 1.2+ protected channel by using {{site.data.keyword.cloud_notm}} Internet Services (powered by Cloudflare). By using Cloud Internet Services there is DDoS protection, a Web Application Firewall, and other network level security mechanisms in place.
* Are encrypted while at rest and in transit to a provider-managed {{site.data.keyword.cloud_notm}} Databases Elastic Search.
* Are securely replicated across the `us-south` and `us-east` regions.
* Are kept for 7 days before they are deleted.


#### security Insights
{: #insights-results-security}

The findings data that is generated by Security Advisor:
...
## Deleting your {{site.data.keyword.compliance_short}} data
{: #data-delete}
