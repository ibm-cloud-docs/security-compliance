...
To increase the security of your system, ensure that there are clear physical and logical boundaries that separate your organization's service delivery and corporate environments.

#### Boundary Protection SC-7-0
{: #boundary-protect-goals}

To better secure your resources, ensure that your information system: 
- monitors and controls communications that happen at the external boundary of the system and at key internal boundaries within the system;
- Implements subnetworks for system components that are publicly accessible to separate them physically or logically from internal organizational networks; 
- Connects to external networks or information systems only through managed interfaces (e.g., gateways, routers, and firewalls) that consist of boundary protection devices that are arranged in accordance with you organization's security architecture.
...
| 3000703 | Check whether App ID redirect URIs are using HTTPS only |   |
| 3000906 | Check whether OpenShift Ingress is configured with at least TLS v1.2 for all inbound traffic |   |
{: caption="Table 48. Cryptographic protection goals" caption-side="top"}

### Cryptographic Key Establishment and Management SC-12
{: #cryptographic-establishment-goals}

Improve the security of your resources by implementing processes for establishing and managing cryptographic keys that are employed within your system, based on your organization-defined requirements for key generation, distribution, storage, access, and destruction. Your organization must define your key management requirements to meet applicable federal laws, Executive Orders, directives, regulations, policies, standards, and guidance, by specifying appropriate options, levels, and parameters.
...
| 3000235 | Check whether Hyper Protect Crypto Services encryption keys that are generated by the service are rotated automatically at least every # months |   |
| 3000903 | Check whether OpenShift cluster has image pull secrets enabled |   |
{: caption="Table 49. Cryptographic keys goals" caption-side="top"}

### Cryptographic Protection SC-13
{: #cryptographic-protection-goals}

To increase the security of your resources, implement the requirements in Supplemental Guidance for Cryptography Governancy that follow applicable federal laws, Executive Orders, directives, policies, regulations, and standards. Your organization must develop an overall cryptographic governance program that meets customer requirements (e.g. hash algorithms used, cryptographic key sizes, operational lifecycles, storage, and distribution methods).
