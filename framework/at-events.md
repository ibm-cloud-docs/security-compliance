...
When **Enforcement** is set to **Off** for a rule, you get an event in the account with the action `compliance.configuration-governance-resource.eval`.

| Field                         |  Details  |
| :----------------------------------| :---------------|
| `initiator`   | Includes information about who or what requested the action. |
| `isAllowed`   | Indicates whether the action was allowed to complete.  |
| `isCompliant` | Indicates whether the action is compliant. |
{: caption="Table 5. Extra information about the resource evaluation action" caption-side="top"}


If the `isAllowed` field is `false` then the severity of the action is *critical*. If the `isAllowed` field is true but the `isCompliant` field is false, then the action severity is *warning*. If both the `isAllowed` and `isCompliant` fields are true, then the severity is *normal*.
