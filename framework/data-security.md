...
You can add a higher level of encryption control to your data at rest (when it is stored) by enabling integration with {{site.data.keyword.keymanagementservicelong_notm}} or {{site.data.keyword.hscrypto}}.The {[s]} [{{site.data.keyword.hscrypto}}](/docs/hs-crypto?topic=hs-crypto-get-started) is a single-tenant key management service that is backed by a FIPS140-2 level 4 certified Hardware Security Model (HSM).
...
All of the data in your account is automatically deleted. You don't need to take any action to delete data that you no longer require. But, depending on the component of {{site.data.keyword.compliance_short}} that you're working with, data is stored and deleted differently.
...
If you delete your account, all of the data that is associated with {{site.data.keyword.compliance_short}} is automatically deleted.
{: note}
