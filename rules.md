...
8. Use the JSON editor to set configuration properties for the rule.
   ...
   | `operator` | The way in which an additional target attribute or property is evaluated against the specified value. For a full list of operators, see [Supported operators](/docs/security-compliance?topic=security-compliance-what-is-rule#rule-operators). |
   ...
   | `required_config` | The requirements that must be met to determine the your resources level of compliance in accordance with the rule.  \n  \nYou can use logical operators (`and`/`or`) to define multiple property checks and conditions. To define requirements for a rule, list one or more property check objects in the `and` array. To add conditions to a property check, use `or`. For more information about defining a rule with multiple conditions, see [How do properties work?](/docs/security-compliance?topic=security-compliance-what-is-rule#rule-properties) |
