...
| {{site.data.keyword.cloud_notm}} Framework for Financial Services (Moderate) | Ensure that the configuration of your customer-managed encryption meet the [{{site.data.keyword.framework-fs_full}} control requirements](/docs/framework-financial-services?topic=framework-financial-services-about#framework-control-requirements) and the baseline requirements that are identified by {{site.data.keyword.cloud_notm}}. | {{site.data.keyword.cloud_notm}} | [![Note icon](../../icons/note_icon.svg)](/docs/security-compliance?topic=security-compliance-moderate-fs-change-log) |
